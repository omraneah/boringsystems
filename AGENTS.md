...
- **Never push with high or critical npm vulnerabilities.** Each npm-capable submodule has a `pre-push` hook running `npm audit --audit-level=high`. Workspace root also runs `.agents/git-hooks/pre-push` which audits every submodule before a pointer bump. Fix path: `npm audit fix` → npm `overrides` → major upgrade → documented advisory acceptance in the affected project's ADR. `--no-verify` forbidden.
- **Harness work goes under `.agents/`, never under `.claude/` or `.codex/`.** Hooks, skills, personas, permissions, git-hooks — anything that should run regardless of which agent is active — lives in `.agents/`. Per-agent setup scripts (`.claude/setup.sh`, `.codex/setup.sh`) wire the agent-agnostic harness into their own runtime. `.claude/` and `.codex/` hold ONLY the per-agent adapters (settings.json, hooks.json, generated persona files). Full rule: `memory/short-term/feedback/stable/feedback_harness_work_under_agents.md`.
...
- **Bash for all scripts.** New standalone scripts MUST be `.sh` — standalone files in other scripting languages (Python, Ruby, Node, etc.) are forbidden without explicit permission. Inline `python3 -c` inside a `.sh` is the **documented escape valve** for the narrow case where bash + jq + printf cannot safely emit JSON (e.g. arbitrary text containing control chars or non-ASCII) — use sparingly, with a code comment explaining why bash alone is unsafe. CLI utility binaries (`jq`, `awk`, `sed`, `grep`, `git`) are tools, not scripting languages, and are encouraged. Full rule + reasoning: `memory/short-term/feedback/stable/feedback_bash_only_scripts.md`.
