...
Everything here must survive a fresh-machine clone + `bash .Codex/setup.sh`. No local-only state, no tokens, no manual MCP setup when a Codex.ai connector exists.

Tests every change must pass:

1. **Fresh-machine.** Clone + setup.sh + `git submodule update --init --recursive` = full working state.
2. **Cloud-agent.** A Codex.ai agent has everything it needs *in the checkout* — skills, docs, config.
3. **No-token.** No `gh auth login`, no API keys, no manual MCP. GitHub/Linear/Gmail via Codex.ai connectors.
...
- `/whence` — directive. Ahmed asks where Codex pulled a claim from.
- `/divergence-check` — proactive. Codex fires on detected frustration / loss-of-fit.
...
|--------|---------|--------|
| `memory/` | Tiered memory: long-term constitutional + identity, medium-term current direction, short-term episodic record. Auto-loaded by Codex via symlink. | Source of truth. Edit via consolidation flow + drift-detection skills. |
...
| `tmp/` | Render buffer — long Codex-generated analysis lands here for Marky reading. NOT for memory: episodic record goes to `memory/short-term/`. | Ephemeral. Folder tracked, contents ignored, wiped at session boundaries. |
...
- **Never open PRs.** Codex pushes; Ahmed opens. No `gh pr create`, no `mcp__github__create_pull_request`.
- **New skills write to the workspace, not the home directory.** Skills always go to `Workspace/.Codex/personal-skills/<name>/SKILL.md`. `~/.Codex/skills/` is a symlink into that directory — never use it as a write target. Before writing to any `~/.Codex/` path, resolve it first (`ls -la`); if it's a symlink into the workspace, write to the workspace path. Full rule: `memory/short-term/feedback/stable/feedback_skills_canonical_path.md`.
- **Connector-first MCP.** Linear, GitHub, Gmail, Notion always via Codex.ai connectors. Never manual auth.
...
- **Long analysis goes to `tmp/`, not chat.** When Codex generates more than ~400 words of dense analysis the user will read in full, write it to `tmp/<name>.md` and reference the path. `tmp/` is ephemeral render buffer; episodic memory goes to `memory/short-term/`. Full rule: `memory/short-term/feedback/in-flight/feedback_tmp_as_ram.md`.
...
- **Linear card lifecycle.** When **creating** a card: end the turn with a 5-bullet executive summary + the card URL + `open <url>` executed via Bash (mirrors the `/pr` end-of-turn shape). When **working** a card: transition to **In Progress** + post a starting comment (branch + bundle + handoff point) when work begins; transition to **In Review** + post an executive summary (delta vs card description, carve-outs, follow-ups) when ready for Ahmed. Cards are durable shared state; chat is ephemeral. **Done** is Ahmed's transition, not Codex's. Full rule: `memory/medium-term/project-management/workspace-workflow.md` § Card creation flow + § Card pickup flow.
- **Retry silently on transient platform errors.** When a buggy CLI / connector / tool error interrupts mid-task ("one million contacts", spurious validation errors, MCP hiccups), retry once same way → retry once differently → route around → only then surface as a single calm sentence. Never let a raw platform-error trace be the user-facing artifact. Full rule: `memory/short-term/feedback/stable/feedback_retry_silently_on_transient_platform_errors.md`.
- **PR push surfaces three artifacts.** Every push that surfaces a `pull/new/<branch>` URL ends the turn with: (1) a 5-bullet concise summary of what shipped, (2) the clickable URL in chat, (3) `open <url>` executed via Bash to launch it in the browser. No asking, every time. Full rule: `memory/medium-term/project-management/workspace-workflow.md` § PR handoff.
