...
- **Bash for all scripts.** New standalone scripts MUST be `.sh`. Python (or any other scripting language) is **forbidden** — including inline `python3 -c` snippets inside `.sh` files. CLI utility binaries (`jq`, `awk`, `sed`, `grep`, `git`) are tools, not scripting languages, and are fine. If a task genuinely needs another language, **stop and ask Ahmed**. Full rule: `memory/short-term/feedback/stable/feedback_bash_only_scripts.md`.
