...
La séparation des préoccupations — le principe que le stockage et le compute doivent appartenir séparément, que ce qui change vite ne doit pas être couplé à ce qui change lentement — est l'un des sept invariants dans *[Engineering Practice Boundaries — Une seule barre pour engineers et AI](/fr/writing/engineering-principles-that-outlive-the-stack)*. Le bet architectural de Neon, c'est ce principe appliqué au provisioning de bases de données.

La base de données éphémère s'accompagne du contexte éphémère : les agents ne persistent pas la donnée qu'ils explorent, et ils ne devraient pas non plus persister le raisonnement scratch qu'ils génèrent. L'architecture pour gérer ce qui *doit* persister — le contexte qui se compose entre sessions — fait l'objet de *[Context is the Edge](/fr/writing/context-is-the-edge)*. La stack complète qui relie tout ça est dans *[The Agent Harness That Runs 80% of My Work](/fr/building/the-harness-i-actually-run)*.
