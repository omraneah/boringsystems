...
- `feedback_card_fanout_discipline.md` (in-flight) — before creating multiple cards, check for container patterns.
- `feedback_linear_card_lifecycle.md` (in-flight) — when working a card, transition status + post comments at start and at handoff.
- `feedback_no_recap_after_link.md` (stable) — once the card exists, the link IS the recap. Don't restate contents in chat.
