...
- `memory/medium-term/project-management/workspace-workflow.md` § Card creation flow + § Card pickup flow — covers the surrounding lifecycle (creation surface, start, done). The self-contained rule governs *what's in the card*; the lifecycle covers *how to surface it*. Both apply at creation.
- `memory/medium-term/project-management/workspace-workflow.md` § PR handoff — the 3-part end-of-turn shape (summary + URL + auto-open in browser) that the card creation surface mirrors.
