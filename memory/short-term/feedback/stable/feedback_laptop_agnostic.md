...
---
**Every decision and every piece of infrastructure must be agent-agnostic and hardware-agnostic.** If a fresh machine clone + the active agent's documented setup path can't reproduce the full working state, the architecture is wrong.
...
1. **Fresh-machine test**: If someone clones the workspace and runs the documented setup for the active agent, does everything work? If not, the change is not done. Fix that agent's setup script or document the missing step.
2. **Cloud-agent test**: If a Claude Code or Codex cloud agent runs against one of these repos, does it have everything it needs *in the repo*? Skills, docs, config, conventions — all in the checkout. No references to a broader local root or untracked global config that the cloud won't see.
...
5. **Agent-boundary test**: Agent-specific runtime setup belongs under that agent's repo folder (`.claude/` for Claude Code, `.codex/` for Codex). Do not make one agent depend on another agent's setup script.
6. **Workspace-scope test**: Setup may configure this workspace checkout. It must not configure `/Users/<user>`, the home directory, or any broader root as a project/workspace just because it is convenient locally.
7. **Symlink hygiene**: Symlinks from `~/` into a git-tracked workspace path are fine when the platform requires them and they are reproducible via setup script. Symlinks in the opposite direction (git → outside) are not.
