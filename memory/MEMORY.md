...
2. **Read every file in `medium-term/feedback/`.** Active behavioural rules — how Claude has been told to operate. Always auto-loads, even though the rest of medium-term is on-demand. Feedback is the live discipline layer.
...
5. **If today is Monday**, follow `medium-term/feedback/feedback_consolidate_week_on_monday_session_start.md` — fire `/consolidate-week` if this week's consolidation hasn't been written yet.
...
`medium-term/feedback/` — active behavioural rules. Auto-loaded (see protocol above). The live discipline layer.
...
- **`/divergence-check`** — proactive. Claude fires on detected frustration / loss-of-fit. Surfaces suspected drift. See `medium-term/feedback/feedback_fire_divergence_check_on_frustration.md` for trigger conditions.

---

## WEEKLY CONSOLIDATION (the closed loop)

- **`/consolidate-week`** — auto-fires Mondays per `medium-term/feedback/feedback_consolidate_week_on_monday_session_start.md`. Reads last week's daily entries, proposes promotions / demotions / drift-flags, Ahmed decides, results recorded in this week's `consolidation.md`.
