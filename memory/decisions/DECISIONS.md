...
- Decision registry: `memory/decisions/DECISIONS.md` — this file
...
**Expected outcome:** A single edit to a persona file, hook, or skill is immediately available to Claude Code (via @file) and available to Codex after generated artifacts are committed. No manual copy-pasting. Fresh-clone + active-agent setup works for either agent. Cloud Codex works from committed artifacts plus Codex's own setup hook.
**Actual outcome:** Superseded/clarified 2026-05-27: Codex runtime setup must not run through `.claude/setup.sh`; Codex-specific setup lives under `.codex/`.

---

## 2026-05-27 — Agent-specific setup boundaries for Claude Code and Codex
**Context:** A Codex runtime approval for browser opening was first accepted as local tool state, then incorrectly routed through `.claude/setup.sh`. That violated the workspace invariant: runtime config must be repo-owned, agent-specific, and reproducible by the agent that needs it.
**Decision:** Claude Code setup stays under `.claude/` and is run by `.claude/hooks/session-start.sh`. Codex setup stays under `.codex/` and is run by `.codex/hooks.json` SessionStart after workspace trust. Codex command approvals live at `.codex/rules/default.rules` and are installed by `.codex/setup.sh`. No agent may depend on another agent's setup script. Codex setup is scoped to this workspace checkout, not a broader user-home root.
**Why:** Agent-agnostic does not mean one shared setup script for every platform. It means shared canonical sources plus clear adapter boundaries per agent. Claude Code and Codex have different runtime directories, hook semantics, and permission mechanisms; mixing them creates hidden coupling and breaks fresh VM onboarding.
**Expected outcome:** A Claude Code VM runs the Claude Code setup path automatically. A Codex VM runs the Codex setup path automatically after project trust. Future agents know to add their own adapter folder instead of editing another agent's setup.
**Actual outcome:** *(pending)*
...
**Decision:** Local merged feature-branch deletion with `git branch -d <feature>` is part of ordinary workspace workflow and is pre-approved. Initial guardrails were narrowed in the following decision: the active boundary is protected-branch push, force-push, and deletion. Enforcement lives in `.agent-hooks/block-protected-push.sh`; policy lives in `.agent-permissions/README.md` and `docs/agent-ops/infrastructure.md`.
