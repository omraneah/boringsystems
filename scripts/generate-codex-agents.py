...
def main():
    ...
    for filename in sorted(os.listdir(CLAUDE_AGENTS_DIR)):
        ...
        # Use TOML literal multiline strings (''') — no escape sequences needed.
        # Personas must not contain '''; verified at time of writing.
        if "'''" in developer_instructions:
            print(f"ERROR: persona {name} contains ''' which breaks TOML literal strings", file=sys.stderr)
            sys.exit(1)

        toml_content = (
            f"# GENERATED — do not edit. Run scripts/generate-codex-agents.py to regenerate.\n"
            f'name = "{name}"\n'
