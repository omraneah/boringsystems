...
- **New skills write to the workspace, not the home directory.** Skills always go to `Workspace/.claude/personal-skills/<name>/SKILL.md`. `~/.claude/skills/` is a symlink into that directory — never use it as a write target. Before writing to any `~/.claude/` path, resolve it first (`ls -la`); if it's a symlink into the workspace, write to the workspace path. Full rule: `memory/short-term/feedback/stable/feedback_skills_canonical_path.md`.
