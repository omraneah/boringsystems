...
- [tmp/ as Workspace Short-Term RAM](feedback_tmp_as_ram.md) — Long Claude-generated analysis goes to `tmp/<name>.md`, referenced by path. Folder tracked, contents ignored, wiped at session boundaries.
- [Render Long Output via /render → Marky](feedback_render_long_output.md) — On "render this with Marky" / "render outside the terminal" trigger, fire `/render` skill — verbatim write to `tmp/<slug>.md` + `marky <path>`. Reader half of the tmp/ loop.
