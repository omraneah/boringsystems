...
One of:
- A slug like `case-files/architecture-governance` or `operating-playbooks/s1-foundations/01-repo-setup` — resolve to the EN file under `boringsystems/src/content/<slug>.md(x)` and the FR file under `boringsystems/src/content/<collection>-fr/<rest>`.
...
Read, in order:
- `boringsystems/docs/target-audiences.md` — persona definitions and IDs.
- `boringsystems/docs/design-charter.md` — voice, typography, layout, anti-patterns.
- `boringsystems/docs/french-guide.md` — FR register and do-not-translate rules.

If any of these is missing, stop and tell Ahmed to check the boringsystems repo — do not proceed on memory.

### 2. Frontmatter checks

- `title` present and non-empty.
- `description` present and non-empty.
- `persona` present. Must be one of `senior-peer`, `operator`, `early-builder`. If absent, this is a **blocker** unless the content predates the schema change (then warning).
...
- **Opening hook.** First paragraph should name the tension. Flag if it reads as "in this article, we will discuss…".
...
- Read the first 3 paragraphs. Check that the entry point matches the persona:
  - `senior-peer` → tension / architecture decision / trade-off named immediately.
  - `operator` → business implication or operational consequence named in paragraph 1.
  - `early-builder` → a decision or stack choice named, with a clear "what to do" orientation.
...
If FR is missing and the persona is `operator` or if the article is in a collection that has existing FR siblings, flag as a warning: "FR version expected but missing."
...
- Internal links use site-relative paths (`/case-files/...`), not full domain URLs.
...
- After running `/new-post` (once that skill exists) — routing hook is documented in `new-post`.
